...
public final class MapleMap {
    ...
    public static int 持有道具不饥饿 = (int) Integer.valueOf(ServerProperties.getProperty("CongMS.持有道具不饥饿"));
    public static int 吸怪盒子 = (int) Integer.valueOf(ServerProperties.getProperty("CongMS.吸怪特权"));
    private short top;
    ...
    private boolean hasCheckIn = false, hasSpawned = false;
    ...
    private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob) {
        ...
        //判断该地图道具数量
        if (mapObjects.get(MapleMapObjectType.ITEM).size() >= CongMS.ConfigValuesMap.get("地图物品上限")) {
            removeDrops();
    ...
    public final void spawnMesoDrop(final int meso, final Point position, final MapleMapObject dropper, final MapleCharacter owner, final boolean playerDrop, final byte droptype) {
        ...
        if (!everlast) {
            int 物品掉落持续时间 = CongMS.ConfigValuesMap.get("物品掉落持续时间");
    ...
    public void getRankAndAdd(final String leader, final String time, final SpeedRunType type, final long timz, final Collection<String> squad) {
        try (final Connection con = (Connection) DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            FileoutputUtil.outError("logs/数据库异常.txt", (Throwable) e);
            e.printStackTrace();
        }
    }
