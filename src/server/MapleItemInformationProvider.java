...
public class MapleItemInformationProvider {
    ...
    public final short getSlotMax(final MapleClient c, final int itemId) {
        ...
        int 额外数量 = CongMS.ConfigValuesMap.get("物品额外数量");
        int wz数量 = 0;
        if (item != null) {
                ...
                switch (scrollId.getItemId()) {
                    ...
                    default: {
                        if (GameConstants.isChaosScroll(scrollId.getItemId())) {
                            ...
                            int stat = ItemConstants.getChaosNumber(scrollId.getItemId());//这里是获取正向加成的数值 0 下面+3 就是3-9 也不麻烦
                            int increase = ItemConstants.isChaosForGoodness(scrollId.getItemId()) || isNegativeScroll(scrollId.getItemId()) ? 1 : Randomizer.nextBoolean() ? 1 : -1;
                            if (nEquip.getStr() > 0) {
                            ...
                            if (nEquip.getWatk() > 0) {
                                nEquip.setWatk((short) (nEquip.getWatk() + (Randomizer.nextInt(stat) + 1) * increase));//下面的你自己加上  就是3-9随机
                            }
                            if (nEquip.getWdef() > 0) {
    ...
    public static void loadFaceHair() {
        try (final Connection con = (Connection) DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            FileoutputUtil.outError("logs/数据库异常.txt", (Throwable) ex2);
        }
    }
