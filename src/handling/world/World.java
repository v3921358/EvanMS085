...
public class World {
    ...
    public static void registerRespawn() {
        WorldTimer.getInstance().register((Runnable) new Respawn(), CongMS.ConfigValuesMap.get("地图刷新频率"));
    }

    public static void registerRespawn1() {//备用刷怪线程
        WorldTimer.getInstance().register((Runnable) new Respawn1(), CongMS.ConfigValuesMap.get("地图刷新频率"));
    }
            ...
            public void run() {
                ...
                if (rate.equals((Object) "经验")) {
                    for (final ChannelServer cservs : ChannelServer.getAllInstances()) {
                        cservs.setExpRate(Double.parseDouble(ServerProperties.getProperty("CongMS.expRate")));
