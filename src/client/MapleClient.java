...
public class MapleClient {
    ...
    final int 角色数量 = ServerConfig.角色数量();
    ...
    public void loadVip(final int accountID) {
        ...
        try (final Connection con = (Connection) DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            try {
                ...
                FileoutputUtil.outError("logs/数据库异常.txt", (Throwable) e2);
            }
        } finally {
            try {
                if (ps != null && !ps.isClosed()) {
    ...
    public final void update2ndPassword() {
        try {
            ...
            FileoutputUtil.outError("logs/数据库异常.txt", (Throwable) ex4);
        }
    }
    ...
    public int getCharacterSlots() {
        ...
        if (charslots != 角色数量) {//读取数据库可创建角色数量
        ...
        try (Connection con = DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            FileoutputUtil.outError("logs/数据库异常.txt", sqlE);
        }
        return charslots;
    }
    ...
    public void setDangerousIp(final String lip) {
        ...
        try (final Connection con = (Connection) DBConPool.getInstance().getDataSource().getConnection()) {
            ...
            FileoutputUtil.outError("logs/数据库异常.txt", (Throwable) Wx);
        }
    }
