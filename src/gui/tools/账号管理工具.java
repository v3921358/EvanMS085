...
public class 账号管理工具 extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        addCanVac1 = new javax.swing.JButton();
        ...
        addCanVac1.setText("授权吸怪");
        addCanVac1.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                addCanVac1ActionPerformed(evt);
            }
        });
        jPanel32.add(addCanVac1, new org.netbeans.lib.awtextra.AbsoluteConstraints(500, 30, -1, 30));
    ...
    private void addCanVac1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_addCanVac1ActionPerformed
        int res = 0;
        String acctxt = 账号.getText();
        if (acctxt == null || acctxt.equals("")) {
            JOptionPane.showMessageDialog(null, "账号内容不能为空");
            return;
        }
        try (Connection dcon = DBConPool.getInstance().getDataSource().getConnection()) {
            PreparedStatement ps = dcon.prepareStatement("UPDATE accounts SET canmobvac = 2 WHERE name = ?");
            ps.setString(1, acctxt);
            res = ps.executeUpdate();
            ps.close();
            if (res > 0) {
                JOptionPane.showMessageDialog(null, "成功授权吸怪账号：" + acctxt);
            } else {
                JOptionPane.showMessageDialog(null, "账号不存在：" + acctxt);
            }
            账号.setText("");
        } catch (SQLException ex) {
            FileoutputUtil.outputFileError("日志/数据库异常.txt", ex);
        }
    }//GEN-LAST:event_addCanVac1ActionPerformed
    private void 刷新家族信息() {
        try {
            Connection con = DatabaseConnection.getConnection();
            PreparedStatement ps = null;
    ...
    private javax.swing.JButton addCanVac1;
