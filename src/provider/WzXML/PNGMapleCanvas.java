...
public class PNGMapleCanvas implements MapleCanvas {
    ...
    public BufferedImage getImage() {
        ...
        try {
            ...
            throw new RuntimeException("zlib fucks", (Throwable) ex);
        }
        dec.end();
        if (this.getFormat() == 1) {
        ...
        final WritableRaster imgRaster = Raster.createWritableRaster(sm, (DataBuffer) imgData, new Point(0, 0));
        final BufferedImage aa = new BufferedImage(this.getWidth(), this.getHeight(), 2);
        aa.setData((Raster) imgRaster);
        return aa;
    }
